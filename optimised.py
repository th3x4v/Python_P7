...
def Sorted_method(actions):
    ...
    sorted_actions = sorted(
        actions, key=lambda x: x["bénéfice_après_2_ans"], reverse=True
    )
    ...
    for action in actions_purchased:
        ...
        print(
            f"Purchased: {action['action']} (Cost: {action['coût_par_action']} euros, Benefit: {action['bénéfice_après_2_ans']})"
        )
...
def Knapsack_method2(actions):
    ...
    for i in range(1, len(actions) + 1):
        for j in range(1, max_cost + 1):
            ...
            if action["coût_par_action"] <= j:
                dp[i][j] = max(
                    dp[i - 1][j],
                    dp[i - 1][j - action["coût_par_action"]]
    ...
    for action in actions_purchased:
        ...
        print(
            f"Purchased: {action['action']} (Cost: {action['coût_par_action']/100} euros, Benefit: {benefit})"
        )

    print(total_benefit)
    print(total_purchase / 100)
    return total_benefit, actions_purchased, total_purchase/100
...
def Knapsack_method(actions):
    ...
    for action in actions:
        for j in range(max_cost, action["coût_par_action"] - 1, -1):
            dp[j] = max(dp[j], dp[j - action["coût_par_action"]] + action["bénéfice_après_2_ans"])

    j = max_cost
    for action in reversed(actions):
        if j - action["coût_par_action"] >= 0 and j - action["coût_par_action"] < len(dp):
            if dp[j] != dp[j - action["coût_par_action"]]:
                actions_purchased.append(action)
                j -= action["coût_par_action"]

    total_purchase = sum(action["coût_par_action"] for action in actions_purchased)
    total_benefit = sum(action["bénéfice_après_2_ans"] for action in actions_purchased)

    for action in actions_purchased:
        print(
            f"Purchased: {action['action']} (Cost: {action['coût_par_action']/100} euros, Benefit: {action['bénéfice_après_2_ans']})"
        )
